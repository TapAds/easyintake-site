/**
 * Renders marketing Terms of Service from the canonical JSON in easy-intake-app.
 * Source: ../../easy-intake-app/packages/shared/src/legal/terms-en.json
 *
 * Outputs: en/terms, terms (alias /terms), es/terms (English body + ES chrome).
 *
 * Run from easy-intake-site: npm run build:terms
 */
...
function renderMainContent(terms) {
  const introHtml = terms.intro
    .map((p) => `<p class="text-gray-400 my-3 leading-relaxed">${escapeHtml(p)}</p>`)
    .join("\n");

  const sectionsHtml = terms.sections
    .map(
  ...
  const contactHtml = `<section class="mt-10">
      <h2 class="text-lg font-semibold text-white">${escapeHtml(terms.contactHeading)}</h2>
      ${renderBlocks(terms.contactBlocks)}
    </section>`;
        ...
        <div class="max-w-7xl mx-auto flex justify-between items-center glass rounded-full px-8 py-3">
            ...
            <div class="hidden md:flex items-center gap-x-8">
                ...
                <div class="flex items-center gap-x-3 text-sm">
                    ...
                    <a href="/es/terms" class="font-medium text-gray-400 hover:text-white transition">ES</a>
                </div>
            </div>
        </div>
    </nav>`;
  ...
  return `<nav class="fixed top-0 w-full z-50 px-6 py-4">
        <div class="max-w-7xl mx-auto flex justify-between items-center glass rounded-full px-8 py-3">
            ...
            <div class="hidden md:flex items-center gap-x-8">
                ...
                <div class="flex items-center gap-x-3 text-sm">
                    <a href="/terms" class="font-medium text-gray-400 hover:text-white transition">EN</a>
                    <span class="text-gray-600">|</span>
  ...
  return `<footer class="border-t border-white/10 py-8 px-6 mt-auto">
        <div class="max-w-7xl mx-auto text-center space-y-4">
            ...
            <div class="flex justify-center items-center gap-3 text-xs text-gray-500">
                <span class="font-medium text-white">EN</span>
                <span class="text-gray-600">|</span>
                <a href="/es/terms" class="hover:text-gray-400">ES</a>
            </div>
...
function footerTermsEs() {
  return `<footer class="border-t border-white/10 py-8 px-6 mt-auto">
        <div class="max-w-7xl mx-auto text-center space-y-4">
            ...
            <div class="flex justify-center items-center gap-3 text-xs text-gray-500">
                <a href="/terms" class="hover:text-gray-400">EN</a>
                <span class="text-gray-600">|</span>
                <span class="font-medium text-white">ES</span>
            </div>
...
function renderPage(terms, opts) {
  const {
    htmlLang,
    canonical,
    hreflangs,
  ...
  const { introHtml, sectionsHtml, contactHtml } = renderMainContent(terms);
...
<head>
    ...
    <meta name="description" content="${escapeHtml(h1Title)} for Easy Intake App (${escapeHtml(terms.productName)})." />
...
<body class="text-gray-200 antialiased min-h-screen flex flex-col">
    ...
    <main class="flex-1 w-full max-w-3xl mx-auto px-6 pt-28 pb-16">
        ...
        ${sectionsHtml}
        ${contactHtml}
        <p class="mt-16 text-sm text-gray-600">${titleSuffix}</p>
    </main>
...
const variants = [
  {
    out: path.join(siteRoot, "terms", "index.html"),
    htmlLang: "en",
    canonical: `${BASE}/terms`,
    hreflangs: [
      { lang: "en", href: `${BASE}/terms` },
      { lang: "es", href: `${BASE}/es/terms` },
      { lang: "x-default", href: `${BASE}/terms` },
    ],
    h1Title: terms.pageTitle,
    subtitle: terms.productName,
    lastUpdated: terms.lastUpdated,
    navHtml: navTermsEn(),
    footerHtml: footerTermsEn(),
    titleSuffix: enCopyright,
    noticeHtml: "",
  },
  {
    out: path.join(siteRoot, "en", "terms", "index.html"),
    htmlLang: "en",
    canonical: `${BASE}/terms`,
    hreflangs: [
      { lang: "en", href: `${BASE}/terms` },
  ...
  {
    out: path.join(siteRoot, "es", "terms", "index.html"),
    htmlLang: "es",
    ...
    h1Title: "Términos del servicio",
    subtitle: terms.productName,
    lastUpdated: terms.lastUpdated,
    navHtml: navTermsEs(),
    footerHtml: footerTermsEs(),
    titleSuffix: "&copy; 2026 Easy Intake App. Todos los derechos reservados.",
    noticeHtml: esNotice,
  },
];

for (const v of variants) {
  const html = renderPage(terms, v);
